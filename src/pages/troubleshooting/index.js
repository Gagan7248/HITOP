...
export default function Troubleshooting() {
  ...
  useEffect(() => {
    fetch('/articles.json', {cache: 'no-store'})
      ...
      .then((data) => {
        const troubleshootingArticles = Array.isArray(data)
          ? data.filter(
              (article) => article.section === 'troubleshooting',
            )
          : [];
      ...
      .catch((error) => {
        console.error(
          'HITOP troubleshooting loading error:',
          error,
        );
  ...
  return (
    <Layout
      ...
      description="HITOP Troubleshooting Knowledge Base">
        ...
        <div className="hitop-articles">
          ...
          {sorted.length === 0 ? (
            <p className="hitop-empty">
              No troubleshooting articles found.
            </p>
          ) : (
            sorted.map((article) => (
              <ArticleCard
